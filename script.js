(function(){
  ...
  function initScrollAnimations() {
    // Respect reduced motion: langsung tampil tanpa animasi
  ...
  function renderMenu(container) {
    ...
    MENU_ITEMS.forEach(item => {
      ...
      if (item.mode) btn.dataset.mode = item.mode;
      if (item.href) {
    ...
    // Indikator menu aktif untuk aksesibilitas
    setActiveMenuIndicator();
  }

  function setActiveMenuIndicator(){
    const mode = document.getElementById('viewCatalog').classList.contains('active') ? 'katalog' : 'preorder';
    [menuCat, menuPO].forEach(menu=>{
      if(!menu) return;
  ...
  function closeAllMenus(){
    [burgerCat,burgerPO].forEach(b=>{ if(b){ b.classList.remove('active'); b.setAttribute('aria-expanded','false'); }});
  ...
  // ========= INITIALIZATION =========
  function init() {
    // --- PERUBAHAN BARU: Menambahkan Anti-Zoom (dengan opsi override) ---
    const allowZoom = localStorage.getItem('allowZoom') === 'true';
    if (!allowZoom) {
