(function () {
  ...
  function applyCapabilityFlags() {
    if (!CSS.supports || !CSS.supports('display', 'grid')) document.body.classList.add('no-grid');
  ...
  function initScrollAnimations() {
    ...
    document.querySelectorAll('.reveal-on-scroll, header.reveal-on-scroll').forEach(el => observer.observe(el));
  }
  ...
  function renderMenu(container) {
    ...
    MENU_ITEMS.forEach(item => {
      ...
      btn.className = 'menu-btn rippleable';
  ...
  function renderList() {
    ...
    for (const it of items) {
      ...
      buttonEl.className = 'list-item rippleable';
  ...
  const poFilterData = () => {
    ...
    return poState.allData.filter(item => {
      if (poState.displayMode === 'detailed') {
        ...
        const status = normalizeStatus(item[2]);
        return match && (statusFilter === 'all' || status === statusFilter);
      }
    });
  };
  ...
  const poRender = () => {
    ...
    if (pageData.length === 0) {
      poList.innerHTML = `<div class="empty">Tidak Ada Hasil Ditemukan</div>`;
  ...
  async function poFetch(sheetName) {
    ...
    try {
      ...
      if (rows.length < 2) { poState.allData = []; return; }
  ...
  function poInit() {
    ...
    poPrev.addEventListener('click', () => {
      if (poState.currentPage > 1) { poState.currentPage--; poRender(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
    });
    poNext.addEventListener('click', () => {
      poState.currentPage++; poRender(); window.scrollTo({ top: 0, behavior: 'smooth' });
    });
  ...
  // ========= RIPPLE (Material-like, opacity 0.16) =========
  function installGlobalRipple() {
    document.addEventListener('click', (e) => {
      const target = e.target.closest('.rippleable, .menu-btn, .pagination-btn, .icon-btn, .wa-button, .list-item');
      if (!target) return;
      if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

      // ensure container has relative overflow hidden
  ...
  function init() {
    ...
    document.addEventListener('click', (e) => {
      const outside = !(menuCat?.contains(e.target) || burgerCat?.contains(e.target) || menuPO?.contains(e.target) || burgerPO?.contains(e.target));
      if (outside) closeAllMenus();
    });
    ...
    // Prepare menus, theme, data, animations
    renderMenu(menuCat);
    renderMenu(menuPO);
    initTheme();
    loadCatalog();
