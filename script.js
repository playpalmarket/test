(function(){
  ...
  // ========= MODE SWITCHER (Navigasi Sidebar) =========
  function setMode(nextMode){
    ...
    document.querySelector(`.sidebar-nav .nav-item[data-mode="${nextMode}"]`).classList.add('active');
  ...
  function poInit(){const rebound=()=>{poState.currentPage=1;poRender();};poSearch.addEventListener('input',rebound);poStatus.addEventListener('change',rebound);poSheet.addEventListener('change',e=>{poFetch(e.target.value==='0'?SHEETS.preorder.name1:SHEETS.preorder.name2);});const mainContent=document.querySelector('.main-content');poPrev.addEventListener('click',()=>{if(poState.currentPage>1){poState.currentPage--;poRender();mainContent.scrollTo({top:0,behavior:'smooth'});}});poNext.addEventListener('click',()=>{poState.currentPage++;poRender();mainContent.scrollTo({top:0,behavior:'smooth'});});poFetch(poSheet.value==='0'?SHEETS.preorder.name1:SHEETS.preorder.name2);poState.initialized=true;}
  ...
  function init() {
    ...
    // Event Listeners
    document.querySelectorAll('.sidebar-nav .nav-item').forEach(item => item.addEventListener('click', (e) => { e.preventDefault(); setMode(item.dataset.mode); }));
